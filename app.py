...
with tab_file:
    ...
    if uploaded_file and st.button("🚀 تشغيل البحث"):
        try:
            ...
            st.error(f"❌ خطأ: {e}")
# ----------------------------------------------------------------------------- #
# 4️⃣ التبويب الرابع: البحث الذكي بالاسم + مركز الاقتراع (إصدار محسَّن وسريع وآمن)
# ----------------------------------------------------------------------------- #
with tab_file_name_center:
    ...
    # تأكد من أن جميع الاستيرادات موجودة في البداية
    from rapidfuzz import process, fuzz
    import time, tempfile, openpyxl, os
    import pandas as pd
    import psycopg2.extras # (مفترض أن get_conn و pd موجودة من مكان آخر في الكود الأصلي)
    ...
    # ملاحظة: يجب أن تكون دالة 'pd' (Pandas) معرفة مسبقًا
    ...
    def load_db_for_centers(centers):
        # ... (الكود الخاص بالتحميل من قاعدة البيانات لم يتم تغييره) ...
        conn = get_conn()
        all_parts = []
        batch_size = 500
    ...
    if file_nc and run_nc:
        ...
        for i, row in df.iterrows():
            orig_name = str(row["الاسم"]) # ضمان التحويل إلى نص
            orig_center = str(row["اسم مركز الاقتراع"]) # ضمان التحويل إلى نص
            ...
            if grp_df is not None and not grp_df.empty:
                ...
                high_match_indices = [j for j, score in enumerate(scores) if score >= 90]
                ...
                if match_count == 1:
                    ...
                    # الحالة 2: أكثر من تطابق بنسبة >= 90%، نحتاج لمركز الاقتراع
                    ...
                    # الحالة 3: لا يوجد تطابق للاسم بنسبة >= 90%
                    best_match = None

                # 4. تعبئة النتيجة النهائية
                if best_match is not None:
                    # إذا كانت النتيجة من الحالة 1 أو 2، استخدمها
                    name_score = best_match["_name_score"]
                    # إذا كان تطابق وحيد، سنحسب نسبة مركز الاقتراع (أو ستكون موجودة من الحالة 2)
                    if match_count == 1:
                        center_score = fuzz.ratio(norm_center, best_match["__norm_center"])
                    else:
                        center_score = best_match["_center_score"]
            ...
            # يتم إضافة السطر إلى النتائج (سواء كان مطابقًا أم لا)
            results.append(match_row)

            # ✅ تحديث واجهة المستخدم والحفظ التدريجي (كما في الكود الأصلي مع تحديث بسيط)
            ...
            if (i + 1) % 100 == 0 or i + 1 == total:
                ...
                # استخدام 'w' للكتابة الكاملة لتجنب مشاكل openpyxl في الوضع 'a' مع الاستبدال
